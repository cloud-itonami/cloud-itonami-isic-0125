(ns berrynutops.operation
  ...
  one supervised actor run. The advisor (BerryNutOpsAdvisor) is sealed
  into a single node (:advise); its proposal is ALWAYS routed through the
  Tree- and Bush-Fruit and Nut Operations Governor (:govern) and the
  ...
  One run = one orchard/grove coordination operation (intake -> advise ->
  govern -> decide -> commit | hold | approval). No unbounded inner loop
  -- each operation is auditable and checkpointed. An orchard/grove
  ...
  hands the decision to a human operator (grower / orchard manager /
  agronomist). The approver resumes the pending request once a decision
  is made. `:flag-crop-health-concern` ALWAYS reaches escalation when the
  Governor is clean -- see `berrynutops.governor/always-escalate-ops`.

  NOTE: langgraph-clj StateGraph integration is deferred (mirrors
  `pomestoneops.operation`, cloud-itonami-isic-0124). This stub version
  ...
  (:require [berrynutops.advisor :as advisor]
...
(defn run-operation
  "Run one tree/bush-fruit-and-nut orchard/grove operation through the
  advisor -> governor -> phase gate -> decision flow. Returns a map with
  ...
  (let [;; Step 1: Advisor proposes
        ...
        disposition-fact (case disposition
                           ...
                           :escalate {:t :approval-requested
                                      :op (:op request) :subject (:orchard-id request)
                                      :reason (or reason
