(ns berrynutops.render-html
  "Build-time HTML renderer for `docs/samples/operator-console.html`.

  Closes flagship checklist item 2 (com-junkawasaki/root ADR-2607189300,
  Wave5). Drives the REAL actor stack (`berrynutops.operation` ->
  `berrynutops.governor` -> `berrynutops.store`) through a scenario adapted
  from this repo's own `berrynutops.sim` demo driver, rendered
...
(def ^:private ctx #(hash-map :actor-id "berry-nut-ops-01" :phase %))

(defn run-demo!
  "Seeds orchard-001 (a berry/nut grove block) plus references an
  unregistered orchard-ghost, then runs every disposition: a crop-health
  concern (phase-2, always escalates -- agronomist approves), a low-risk
  field-operation schedule (phase-1 auto-commit), a phase-0 sandbox
  schedule that escalates (operator approves), and a log-orchard-record
  against the unregistered orchard-ghost (HARD block). No invented values."
  []
  (let [db (store/mem-store
            {:initial-orchards
             {"orchard-001" {:id "orchard-001" :name "Test Grove Block" :fruit-class "blueberry"}}})
        actor (op/build db)]
    (exec! actor "t1" {:op :flag-crop-health-concern :orchard-id "orchard-001"
                       :concern "spotted-wing-drosophila-suspected"} (ctx :phase-2))
    (approve! actor "t1" "agronomist-01")
    (exec! actor "t2" {:op :schedule-field-operation :orchard-id "orchard-001"
                       :requested-date "2026-08-01"} (ctx :phase-1))
    (exec! actor "t3" {:op :schedule-field-operation :orchard-id "orchard-001"
                       :requested-date "2026-09-01"} (ctx :phase-0))
    (approve! actor "t3" "berry-nut-ops-01")
...
(defn- status-cell [ledger oid]
  (let [f (last-fact-for ledger oid)]
    (cond
      ...
      (= :governor-hold (:t f)) (str "<span class=\"critical\">HARD hold &middot; " (esc (name (or (hold-rule f) :unknown))) "</span>")
...
(def ^:private action-gate-rows
  ["        <tr><td><code>:flag-crop-health-concern</code></td><td><span class=\"warn\">ALWAYS human approval (crop safety)</span></td></tr>"
   ...
   "        <tr><td><code>:log-orchard-record</code></td><td><span class=\"warn\">registered-orchard required; else HARD block</span></td></tr>"])
(defn render [db]
  (let [ledger (vec (store/ledger db))
        o-ids ["orchard-001" "orchard-ghost"]
        orow (fn [oid] (let [o (store/registered-orchard db oid)]
    ...
    (str
     "<html><head><meta charset=\"utf-8\"><title>cloud-itonami-isic-0125 &middot; berry/nut growing ops</title><style>"
     "body{font:14px/1.5 -apple-system,system-ui,sans-serif;margin:0;color:#1a1a1a;background:#f5f5f5}"
     ".bar{background:#2a0a3a;color:#fff;padding:1.2rem 2rem}.bar h1{margin:0;font-size:1.15rem;font-weight:600}"
     ...
     "</style></head><body>\n"
     "<header class=\"bar\">\n  <h1>Berry/nut growing ops (ISIC 0125) — Operator Console</h1>\n"
     ...
     "<main>\n  <section class=\"card\">\n    <h2>Scenario grove blocks</h2>\n"
     "    <p class=\"muted\">Demo snapshot — build-time-generated from <code>berrynutops.store</code> via <code>berrynutops.render-html</code> (<code>clojure -M:dev:render-html</code>), regenerated nightly. No invented data.</p>\n"
     "    <table>\n      <thead><tr><th>Orchard</th><th>Name</th><th>Fruit class</th><th>Last op status</th></tr></thead>\n      <tbody>\n"
     orchard-rows "\n      </tbody>\n    </table>\n  </section>\n"
     "  <section class=\"card\">\n    <h2>Action gate (BerryNutOps Governor)</h2>\n"
     "    <p class=\"muted\">HARD blocks cannot be overridden. Unregistered orchards are rejected before any human; phase-0 (sandbox) never auto-commits.</p>\n"
