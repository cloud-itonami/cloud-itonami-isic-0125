(ns berrynutops.phase
  "Phase gate for the tree- and bush-fruit and nut orchard/grove
  operations coordinator rollout (0->3 maturity). Mirrors
  `pomestoneops.phase` (cloud-itonami-isic-0124) / `citrusops.phase`
  (cloud-itonami-isic-0123) in shape, adapted to this actor's simpler op
  set (no separate request-level :stake -- the request's :op IS the
  stake, since every op this actor may propose is already
  coordination-only).
  ...
  Phase 1: supervised operation -- always-escalate ops
    (`berrynutops.governor/always-escalate-ops`) escalate even when
  ...
  (:require [berrynutops.governor :as governor]))
